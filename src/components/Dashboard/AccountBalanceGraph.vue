<template>
    <LaCartesian :data="transactions" autoresize>
        <LaLine v-for="account in $auth.user.accounts"
                :key="account.id"
                :prop="account.name"
                :label="account.name"
                :width="2"
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            const first_date = this.date_buffer[0];
            ...
            for (let i = 0; i < this.date_buffer.length; i++) {
                ...
                this.$auth.user.accounts.forEach(account => {
                    const transactions_on_date = account.transactions.filter(transaction => {
                        const transaction_date = transaction.created.split('T')[0];
                    ...
                    if (amount) {
                        ...
                        const previous_date = this.date_amounts_buffer[this.date_buffer[i - 1]];
                        if (previous_date) {
            ...
            console.log('don\'t look at the errors')
        },
        methods: {
            randomColor() {
